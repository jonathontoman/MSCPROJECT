...
public class GraphValidator 
{
				...
				if (outboundEdges < 1) {
					System.err
							.println("Found Start Node with no outbound edges in piste "
									+ n.getPistes());
					System.err.println(OutputKML.outputPlaceMark(n.getLongitude(), n.getLatitude()));
				}
			}
	...
	public static void validateNodes(LinkedList<Node> nodes)
	{
		for (Node n : nodes) {
			int outboundEdges = n.getOutbound().size();
			int inboundEdges = n.getInbound().size();
	...
	public static void validatePistes(Set<Piste> pistes)
	{
		for (Piste p  : pistes)
		{
			validateNodes(p.getNodes());
		}
	}
}
